...
//    public ArrayList<Player> players = new ArrayList<>();
    ...
    public boolean isHoiSinhSuperZou = false;
...
//                    msg.writer().write(effItem);
                ...
                if(pl.taskId == (short)8 && pl.crrTask.index == (byte)3 && map.id == 47) {
                    if(pl.power < (pl.getPowerLimit()*1000000L)) {
...
//            if ((((Math.abs(p.x - mob.pointX) <= xSkill) && (Math.abs(p.y - mob.pointY) <= ySkill)) || (mobId == (byte) 70)) && !p.isTroi) {
                ...
                if (p.idSkillselect == 20 && p.mp >= skillPlayerUse.manaUse && !p.isdie && !mob.isDie && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown))) {
                    ...
                    } else if (p.power >= 10000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 18);
                    }
...
//                    }
                ...
                if (p.mp >= kiUse && !p.isdie && !mob.isDie && mob.hp > 0 && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown)) && (_timeNOW >= p.timeCanSkill)) {
                    ...
                    if (p.getSetKichHoatFull() == (byte) 3 && skillPlayerUse.template.id == (byte) 1) {
                        ...
                        damage = damage * 2;
                    } else if (p.getSetKichHoatFull() == (byte) 10 && skillPlayerUse.template.id == (byte) 17) { //SET KICH HOAT LH
                        damage = damage * 2;
                    }
...
//                        item.itemTemplateID = (short) item.itemMapID;
                            ...
                            if (mob.template.tempId != 0) {
//                            int idItemNotSell[] = {14,15,16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 85, 993};
                                int idItemNotSell[] = {18,19,20, 441,442,443,444,445,446,447, 76,188,189,993,949,851};
                                int percentDrop = Util.nextInt(0, 17);
//                            int percentDropItemGod = Util.nextInt(0, 10000);
//                            int percentDropKichHoat = Util.nextInt(0, 5000);
                                int percentDropItemGod = Util.nextInt(0, 40000);
                                int percentDropKichHoat = Util.nextInt(0, 40000);
                                int perTA = Util.nextInt(0, 5000);
//                                if (percentDropKichHoat < 5 && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
                                if (percentDropKichHoat < 50 && p.checkFullSetThienSu() && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
                                    int id = Util.nextInt(0, 5); //RANDOM TU 0,1,2,3,4
                                    int idItemKichHoat[][] = {{0, 6, 21, 27, 12}, {1, 7, 22, 28, 12}, {2, 8, 23, 29, 12}};
                                    int optionItemKichHoat[][] = {{127, 128, 129, 139, 140, 141}, {130, 215, 132, 142, 216, 144}, {133, 134, 135, 136, 137, 138}};
                                  ...
                                  itemROI.itemTemplateID = (short)itemROI.itemMapID;
                                    ...
                                    if (percentDrop < 5 && map.MapCell() && p.useMayDoCapsule) { //ITEM ROT KHI DUNG RADA DO CAPSULE
//                                int idItemNotSell2[] = {16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225,16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 380};
                                    int idItemNotSell2[] = {16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225,16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 380};
                                    int id = Util.nextInt(0, idItemNotSell2.length);
                                    if (id >= 27) {
                                        id = 31;
                                    }
...
//                                } else if (percentDropItemGod < 5 && map.MapCold()) {
...
//                                }
                            ...
                            int percentDrop2 = Util.nextInt(0, 3);
                            if (mob.template.tempId != 0 && percentDrop2 < 3) {
                                int idItemNotSell2[] = {85 , 20, 993};
                                int id2 = Util.nextInt(0, 4);
    ...
    //            }
                ...
                if (_bTarget.isdie && _bTarget.typePk == 5) {
                    ...
                    if(_bTarget._typeBoss != 1 && _bTarget._typeBoss != 2) { //Broly khong roi do
                        ArrayList<ItemMap> itemDrop = new ArrayList<>();
                        if(_bTarget._typeBoss == 3 || _bTarget._typeBoss == 5 || (_bTarget._typeBoss >= (byte)7 && _bTarget._typeBoss <= (byte)24)) {
                        ...
                        } else if(_bTarget._typeBoss == 52 || _bTarget._typeBoss == 54 || _bTarget._typeBoss == 49 && _bTarget._typeBoss == 50) {
                            Service.gI().sendThongBaoServer(p.name + " vừa tiêu diệt " + _bTarget.name + " mọi người đều ngưỡng mộ");
                        ...
                        } else if(_bTarget._typeBoss == 53) {
                            Service.gI().sendThongBaoServer(p.name + " vừa tiêu diệt " + _bTarget.name + " mọi người đều ngưỡng mộ");
                            //ADD ITEM TO MAP
                            if(Util.nextInt(0, 1) == 0) {
                                ItemMap itemM = newItemMAP(1098, p.id, _bTarget.x, _bTarget.y);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                }
                            }
                            ...
                            if(Util.nextInt(0, 8) < 5) {
                                ItemMap itemM = newItemMAP(1009, p.id, _bTarget.x, _bTarget.y);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                }
                            }
                        ...
                        } else if(_bTarget._typeBoss == (byte)25) {//kingkong rot 4s
                            ItemMap itemM = newItemMAP(17, p.id, _bTarget.x, _bTarget.y);
                            if(itemM != null) {
                                addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                            }
                        } else if(_bTarget._typeBoss == (byte)28) {//cell rot 3s
...
//                    Util.log("DAME LAZE: " + damage);
                ...
                if (_bTarget.isdie && _bTarget.typePk == 5) {
                    ...
                    if(_bTarget._typeBoss != 1 && _bTarget._typeBoss != 2) { //Broly khong roi do
                        ArrayList<ItemMap> itemDrop = new ArrayList<>();
                        if(_bTarget._typeBoss == 3 || _bTarget._typeBoss == 5 || (_bTarget._typeBoss >= (byte)7 && _bTarget._typeBoss <= (byte)24 && _bTarget._typeBoss != (byte)29)) {
                            if(_bTarget._typeBoss == (byte)7) {
                                Server.gI().mapKUKU = 0;
                                Server.gI().khuKUKU = 0;
                            } else if(_bTarget._typeBoss == (byte)8) {
...
//                                itemROI.itemTemplateID = (short)itemROI.itemMapID;
    ...
    //                                itemGOD.item.template = ItemTemplate.ItemTemplateID(_ITEMMAPID);
                        ...
                        } else if(_bTarget._typeBoss == 52 || _bTarget._typeBoss == 49 || _bTarget._typeBoss == 50 || _bTarget._typeBoss == 54) {
                            Service.gI().sendThongBaoServer(p.name + " vừa tiêu diệt " + _bTarget.name + " mọi người đều ngưỡng mộ");
                            ItemMap item = dropItemGOD(p, _bTarget.x, _bTarget.y);
...
if(_bTarget._typeBoss == 51){
ItemMap itemM = newItemMAP(568, p.id, _bTarget.x, _bTarget.y);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                }
                        ...
                        if (Math.abs(player.x - mob.pointX) < templateSkillUse.dx && Math.abs(player.y - mob.pointY) < templateSkillUse.dy && !mob.isDie) {
                                ...
                                if(mob.isDie) {
                                    ...
                                    if(mob.template.tempId != 0) {
                                        ...
                                        int percentDrop = Util.nextInt(0, 17);
                                        if(percentDrop < 5) {
                                            int id = Util.nextInt(0, idItemNotSell.length);
                        //                    Item itemMap = ItemSell.getItem(id);
                                            Item itemMap = ItemSell.getItemNotSell(idItemNotSell[id]);
                                            ItemMap item = new ItemMap();
                            ...
                            if (Math.abs(player.x - bossMap.get(i).x) < templateSkillUse.dx && Math.abs(player.y - bossMap.get(i).y) < templateSkillUse.dy && !bossMap.get(i).isdie && Service.gI().checkCanAttackBoss(bossMap.get(i))) {
                                ...
                                if (bossMap.get(i).isdie && bossMap.get(i).typePk == 5) {
                                    ...
                                    if(bossMap.get(i)._typeBoss != 1 && bossMap.get(i)._typeBoss != 2) { //Broly khong roi do
                                        ...
                                        if(bossMap.get(i)._typeBoss == 3 || bossMap.get(i)._typeBoss == 5 || (bossMap.get(i)._typeBoss >= (byte)7 && bossMap.get(i)._typeBoss <= (byte)24 && bossMap.get(i)._typeBoss != (byte)29)) {
...
//                                                itemROI.itemTemplateID = (short)itemROI.itemMapID;
                    ...
                    //                                itemGOD.item.template = ItemTemplate.ItemTemplateID(_ITEMMAPID);
                                        ...
                                        } else if(bossMap.get(i)._typeBoss == 52 || bossMap.get(i)._typeBoss == 49 || bossMap.get(i)._typeBoss == 50 || bossMap.get(i)._typeBoss == 54) {
                                            Service.gI().sendThongBaoServer(player.name + " vừa tiêu diệt " + bossMap.get(i).name + " mọi người đều ngưỡng mộ");
                                        ...
                                        } else if(bossMap.get(i)._typeBoss == 56) {
                                            Service.gI().sendThongBaoServer(player.name + " vừa tiêu diệt " + bossMap.get(i).name + " mọi người đều ngưỡng mộ");
                                            ItemMap item = dropItemGOD(player, bossMap.get(i).x, bossMap.get(i).y);
                                            ...
                                            if(Util.nextInt(0, 8) < 5) {
                                                ItemMap itemM = newItemMAP(1009, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                                if(itemM != null) {
                                                    addItemToMap(itemM, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                                }
                                            }
                                        } else if(bossMap.get(i)._typeBoss == 53) {
                                            Service.gI().sendThongBaoServer(player.name + " vừa tiêu diệt " + bossMap.get(i).name + " mọi người đều ngưỡng mộ");
                                            if(Util.nextInt(0, 1) == 0) {
                                                ItemMap itemM = newItemMAP(1098, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                                if(itemM != null) {
                                                    addItemToMap(itemM, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                                }
                                            }
                                        ...
                                        } else if(bossMap.get(i)._typeBoss == (byte)25) {//kingkong rot 4s
                                            ItemMap itemM = newItemMAP(17, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                            if(itemM != null) {
                                                addItemToMap(itemM, player.id, bossMap.get(i).x, bossMap.get(i).y);
                                            }
                                        } else if(bossMap.get(i)._typeBoss == (byte)28) {//cell rot 3s
                                            if(Util.nextInt(0, 10) <= 7) {
        ...
        if(mapid != 84 && mapid != 85 && mapid != 86 && mapid != 87 && mapid != 88 && mapid != 89 && mapid != 90 && mapid != 91 && mapid != 102 && mapid != 114 && mapid != 115 && mapid != 116 && mapid != 117
         ...
         && mapid != 152 && mapid != 131 && mapid != 132 && mapid != 133) {
            if(p.taskId < (short)8 && ((mapid >= 45 && mapid <= 50) || mapid == 12 || mapid == 18)) {
                ...
                Service.gI().buyDone(p);
                return;
            } else if(p.taskId < (short)32 && mapid >= 161) {
                p.sendAddchatYellow("Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                Service.gI().buyDone(p);
                return;
            }
        ...
        if(type == (byte)0) {
            ...
            if(templateSkillUse != null) {
                ...
                if(player.mp >= _manaUse && !player.checkPlayerBiKhongChe() && ((_TIMENOW - templateSkillUse.lastTimeUseThisSkill) > (long)templateSkillUse.coolDown)) {
                    ...
                    player.mp -= _manaUse;
                    player.dameChim = 15 + templateSkillUse.point*5;
                    Message m = null;
                    try {
                        m = new Message(-95);
...
//                                                    }
                                    ...
                                    chat(_detu, "Ê cu cho a 1 hơi đê!");
                                }
                            }
                        }
                    }
    ...
    public void PetSendAttack(Player _player, Detu _detu, Mob _mob, Skill _skill) throws IOException {
        ...
        if(_detu.getSetKichHoatFull() == (byte)3 && _skill.template.id == (byte)1) {
            ...
            damage = damage*2;
        }else if(_detu.getSetKichHoatFull() == (byte)10 && _skill.template.id == (byte)17) { //SET KICH HOAT LH CUA DE TU
            damage = damage*2;
        }
...
//        long upSmTn = (long)(damage*20);
        ...
        if(_detu.power > 40000000000L) {
            ...
            upSmTn = (long)(upSmTn/7);
        } else if(_detu.power > 80000000000L) {
            upSmTn = (long)(upSmTn/10);
        } else if(_detu.power > 100000000000L) {
...
//                openSkillPetAndUpPoint(_detu);
            ...
            if(_player.power < (_player.getPowerLimit()*1000000000L)) {
                ...
                _player.UpdateSMTN((byte) 2, (long)(upSmTn/6)); // send cong tnsm cho su phu khi de danh
                if(_player.clan != null) {
...
//                                itemGOD.item.template = ItemTemplate.ItemTemplateID(_ITEMMAPID);
                        ...
                        if(_ITEMGOD.template.id == 555 || _ITEMGOD.template.id == 557 || _ITEMGOD.template.id == 559) { //AO THAN
                            ...
                            _ITEMGOD.itemOptions.add(new ItemOption(14, Util.nextInt(9, 16)));
                        }
                        _ITEMGOD.itemOptions.add(new ItemOption(21, 17));
                        itemROI.item = _ITEMGOD;
                        itemDrop.add(itemROI);
    ...
    public void openSkillPetAndUpPoint(Detu _detu) {
        ...
        if(_detu.power >= 20000000000L && _detu.listSkill.get(3).skillId == -1) {
            ...
            if(rdSkill2 == 0) {
                ...
                _detu.listSkill.get(3).skillId = 84; //id khien nang luong lv1
                _detu.listSkill.get(3).point = 1;
                _detu.listSkill.get(3).genderSkill = (byte)1;
                _detu.listSkill.get(3).tempSkillId = 12;
            } else {
...
//                                    _broly.hpFull = (int)(_broly.hpFull*2);
    ...
    //                        }
     ...
     public void loadZou(Boss _broly) {
        haveBROLY = true; //check xem map da co broly hay chua
        Message msg;
        try {
            loadInfoBoss(_broly); //tha broly theo server roi khong can ve cho all player trong map nua
                            ...
                            Timer timerBrolyBack = new Timer();
                                TimerTask ttBrolyBack = new TimerTask() {
                                    public void run()
                                    {
                                        ...
                                        Boss _rBroly = new Boss(numberBoss, (byte)51, (short)Util.getToaDoXBROLY(Server.gI().idMapBroly[idMap]), (short)Util.getToaDoYBROLY(Server.gI().idMapBroly[idMap]));
                                        Server.gI().maps[Server.gI().idMapBroly[idMap]].area[_rdZone].bossMap.add(_rBroly);
                                        Server.gI().maps[Server.gI().idMapBroly[idMap]].area[_rdZone].loadBROLY(_rBroly);
                                    };
                                };
...
//                        timerBOSS.cancel();
    ...
    //                        haveBROLY = false;
                            if(_BOSS._typeBoss == 3) { //COOLER 1 DIE
                                ...
                                timerCoolerBack.schedule(ttCoolerBack, 5000);
                                ...
                                timerCoolerXuatHien.schedule(ttCoolerXuatHien, 900000);
                                ...
                                timerBLACKBack.schedule(ttBLACKBack, 5000);
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 900000);
                            } else if(_BOSS._typeBoss == 55) { // Cumber Die
                                Timer timerBLACKBack = new Timer();
                                TimerTask ttBLACKBack = new TimerTask() {
                                    public void run()
                                    {
                                        Boss _rBlack = new Boss(536, (byte)56, xDIE, yDIE);
                                        bossMap.add(_rBlack);
                                        loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKBack.schedule(ttBLACKBack, 5000);
                            } else if(_BOSS._typeBoss == 56) { // SUPER CUMBER DIE
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        int idMap = 151; //index 91 la map 92, index 92 la map 93
                                        int IDZONE = Util.nextInt(0, map.area.length);
                                        short xBlack = 228;
                                        if(idMap == 151) {
                                            xBlack = 1296;
                                        }
                                        Boss _rBlack = new Boss(535, (byte)55, xBlack, (short)360);
                                        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                                        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 900000);
                            }else if(_BOSS._typeBoss == 54) { // Fidegold
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        ...
                                        Boss _rBlack = new Boss(534, (byte)54, xBlack, (short)360);
                                        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                                        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 1200000);
                            }else if(_BOSS._typeBoss == 53) { // granolai
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        ...
                                        Boss _rBlack = new Boss(102, (byte)53, xBlack, (short)360);
                                        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                                        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 600000);
                            }else if(_BOSS._typeBoss == 53) { // granolai 1
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 600000);
                            }else if(_BOSS._typeBoss == 53) { // granolai 2
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 600000);
                            }else if(_BOSS._typeBoss == 52) { // coolergold
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        ...
                                        Boss _rBlack = new Boss(532, (byte)52, xBlack, (short)360);
                                        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                                        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 1200000);
                            }else if(_BOSS._typeBoss == 51) { // Zou 1
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        ...
                                        Boss _rBlack = new Boss(102, (byte)51, xBlack, (short)360);
                                        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                                        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                                    };
                                };
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 51) { // Zou 2
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 51) { // Zou 3
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 120000);
    ...
    //                            Boss _So0 = getBossByID(-200129);
                                ...
                                timerFide.schedule(ttFide, 5000);
    ...
    //                                    loadBossNoPet(_SBH2);
                                ...
                                timerYcon.schedule(ttYcon, 60000);
    ...
    //                            timerMabu.schedule(ttMabu, 0, 1000);
                                timerMabu.schedule(ttMabu, 60000);
    ...
    //                            itemM.itemTemplateID = (short) itemM.itemMapID;
                                ...
                                Item itemMap = new Item(lychee.item);
                                itemMap.itemOptions.clear();
                                int maxP = (int)Math.ceil(51*_BOSS.lvGas/110);
                                maxP = maxP > 3 && maxP <= 51 ? maxP : 3;
                                itemMap.itemOptions.add(new ItemOption(50, Util.nextInt(1, maxP)));
                                itemMap.itemOptions.add(new ItemOption(77, Util.nextInt(1, maxP)));
                                itemMap.itemOptions.add(new ItemOption(103, Util.nextInt(1, maxP)));
                                itemMap.itemOptions.add(new ItemOption(5, Util.nextInt(1, 36)));
                                itemMap.itemOptions.add(new ItemOption(30, 0)); //KHONG THE GIAO DICH
                                int dayEx = Util.nextInt(1, 21);
                                itemMap.itemOptions.add(new ItemOption(93, dayEx));
                                itemMap.timeHSD = System.currentTimeMillis() + (long)dayEx*86400000;
    ...
    //                            itemM.itemTemplateID = (short) itemM.itemMapID;
                                ...
                                timerZamasu.schedule(ttZamasu, 900000);
...
//                    Player _charTarget = getCharNearest(_BOSS.x, _BOSS.y, 2000); //get char gan nhat
    ...
    //                                if(_broly.countCharge >= 10 || (_broly.hp >= _broly.hpFull)) {
                                    if((long)_BOSS.hp >= (hpBossNow + (long)((_BOSS.hpFull)*0.1)) || _BOSS.hp >= _BOSS.hpFull || _BOSS.checkBiKhongChe() || _BOSS.isdie || _BOSS.countCharge >= 10) {
                                        ...
                                        chat(_BOSS, "Chờ tí anh tái tạo đã!");
                                        }
                                    }
                                };
                            };
                    ...
                    } else if((_BOSS._typeBoss > (byte)2 && _BOSS._typeBoss < (byte)7) || (_BOSS._typeBoss >= (byte)26 && _BOSS._typeBoss != (byte)29 && _BOSS._typeBoss <= (byte)30) || _BOSS._typeBoss >= (byte)48) {
                        long timeNow = System.currentTimeMillis();
                        if((timeNow - _BOSS.lastTimeUseChargeSkill) > 1 && _BOSS.mp < _BOSS.mpFull && !_BOSS.isBlind && !_BOSS.checkBiKhongChe()) {
                            _BOSS.lastTimeUseChargeSkill = timeNow;
                            _BOSS.isBlind = true;
                            _BOSS.bossNoPetChargeHPMP(_BOSS, 1);
                            // creating timertask, timer
                            long mpBossNow = (long)_BOSS.mp;
                            Timer timerPetTTNL = new Timer();
                            TimerTask petTTNL = new TimerTask() {
                                public void run()
                                {
    //                                if(_broly.countCharge >= 10 || (_broly.hp >= _broly.hpFull)) {
                                    if((long)_BOSS.mp >= (mpBossNow + (long)((_BOSS.mpFull)*0.1)) || _BOSS.mp >= _BOSS.mpFull || _BOSS.checkBiKhongChe() || _BOSS.isdie || _BOSS.countCharge >= 10) {
                                        _BOSS.countCharge = 0;
                                        _BOSS.isBlind = false;
                                        _BOSS.bossNoPetChargeHPMP(_BOSS, 3);
                                        timerPetTTNL.cancel();
                    ...
                    } else if  (_BOSS._typeBoss == (byte)54 || _BOSS._typeBoss == (byte)29 || _BOSS._typeBoss == (byte)52) {
                        if(!_BOSS.isBoom) {
                            _BOSS.isBoom = true;
...
//                                }
                    ...
                    if(_charTarget != null) {
                        if(_BOSS._typeBoss < (byte)7) {
                            ...
                            chat(_BOSS, "bruh Bruh dark dark lmao lmao!");
                        } else if(_BOSS._typeBoss == (byte)52) {
                            chat(_BOSS, "Không làm mà đòi có ăn,thì ăn đầu buồi!");
                        } else if(_BOSS._typeBoss == (byte)54) {
                            chat(_BOSS, "Ra xã Hội làm ăn bươn trải!");
                            chat(_BOSS, "Liều thì ăn Nhiều,Không liều thì ăn ít!");
                        } else if(_BOSS._typeBoss == (byte)53) {
                            chat(_BOSS, "Haha Vào Đây mà Hốc!");
                            chat(_BOSS, "Ăn được tao?tao cho hoa hồng blue!");
                            chat(_BOSS, "Nhót!");
                        } else if(_BOSS._typeBoss == (byte)55 || _BOSS._typeBoss == (byte)56) {
                            chat(_BOSS, "Tôi Năm Nay Hơn 70 tuổi rồi\bChưa gặp trường hợp nào như này cả!");
                            chat(_BOSS, "Địt mẹ. mấy thằng ranh con này\b tao vả cho mấy cái giờ!");
                        }
                    }
                }
            };
...
//        short minSpace = 150;
    ...
    public ItemMap dropItemGOD(Player p, short xDROP, short yDROP) {
        ...
        //            itemsMap.addAll(itemDrop);
                ...
                int idNRRot = Util.nextInt(15,16);
