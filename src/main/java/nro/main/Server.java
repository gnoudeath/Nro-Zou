        ...
        try {
            ...
            for(int i = 0; i < idMapBroly.length; i++) {
                ...
                for(int j = 0; j < 2; j++) {
                    ...
                    maps[idMapBroly[i]].area[_rdZone].loadZou(_sBroly);
...
//                    Util.log("MAP NAME: " + maps[idMapBroly[i]].template.name + ", khu: " + _rdZone);
            ...
            Timer timerGranolaiXuatHien = new Timer();
            TimerTask ttGranolaiXuatHien = new TimerTask() {
                public void run()
                {
                    int idMap = 0;//Util.nextInt(91, 93); //index 91 la map 92, index 92 la map 93
                    int IDZONE = Util.nextInt(0, maps[idMap].area.length);
                    short xGranolai = 228;
                    if(idMap == 0) {
                        xGranolai = 1296;
                    }
                    Boss _rGranolai = new Boss(102, (byte)53, xGranolai, (short)360);
                    maps[idMap].area[IDZONE].bossMap.add(_rGranolai);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rGranolai);
                    Util.log("INIT Granolai XONG KHU " + IDZONE);
                    timerGranolaiXuatHien.cancel();
                };
            };
            timerGranolaiXuatHien.schedule(ttGranolaiXuatHien, 30000);
            
            Timer timerGranolai1XuatHien = new Timer();
            TimerTask ttGranolai1XuatHien = new TimerTask() {
                public void run()
                {
                    int idMap = 7;//Util.nextInt(91, 93); //index 91 la map 92, index 92 la map 93
                    int IDZONE = Util.nextInt(0, maps[idMap].area.length);
                    short xGranolai = 228;
                    if(idMap == 0) {
                        xGranolai = 1296;
                    ...
                    maps[idMap].area[IDZONE].bossMap.add(_rGranolai);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rGranolai);
                    Util.log("INIT Granolai1 XONG KHU " + IDZONE);
                    timerGranolai1XuatHien.cancel();
                };
            };
            timerGranolai1XuatHien.schedule(ttGranolai1XuatHien, 30000);
            
            Timer timerGranolai2XuatHien = new Timer();
            TimerTask ttGranolai2XuatHien = new TimerTask() {
                public void run()
                {
                    int idMap = 14;//Util.nextInt(91, 93); //index 91 la map 92, index 92 la map 93
                    int IDZONE = Util.nextInt(0, maps[idMap].area.length);
                    short xGranolai = 228;
                    if(idMap == 14) {
                        xGranolai = 1296;
                    ...
                    maps[idMap].area[IDZONE].bossMap.add(_rGranolai);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rGranolai);
                    Util.log("INIT Granolai2 XONG KHU " + IDZONE);
                    timerGranolai2XuatHien.cancel();
                };
            };
            timerGranolai2XuatHien.schedule(ttGranolai2XuatHien, 30000);
            
            
            Timer timerCoolergoldXuatHien = new Timer();
            TimerTask ttCoolergoldXuatHien = new TimerTask() {
                public void run()
                {
                    int idMap = 6;//Util.nextInt(91, 93); //index 91 la map 92, index 92 la map 93
                    int IDZONE = Util.nextInt(0, maps[idMap].area.length);
                    short xBlack = 228;
                    if(idMap == 91) {
                        xBlack = 1296;
                    }
                    Boss _rBlack = new Boss(102, (byte)52, xBlack, (short)360);
                    maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                    Util.log("INIT CoolerGold XONG KHU " + IDZONE);
                    timerBLACKXuatHien.cancel();
                };
            };
            timerCoolergoldXuatHien.schedule(ttCoolergoldXuatHien, 30000);
            
            Timer timerFidegoldXuatHien = new Timer();
            TimerTask ttFidegoldXuatHien = new TimerTask() {
                public void run()
                {
                    int idMap = 24;//Util.nextInt(91, 93); //index 91 la map 92, index 92 la map 93
                    int IDZONE = Util.nextInt(0, maps[idMap].area.length);
                    short xBlack = 228;
                    if(idMap == 91) {
                        xBlack = 1296;
                    }
                    Boss _rBlack = new Boss(102, (byte)54, xBlack, (short)360);
                    maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                    Util.log("INIT FideGold XONG KHU " + IDZONE);
                    timerBLACKXuatHien.cancel();
                };
            };
            timerFidegoldXuatHien.schedule(ttFidegoldXuatHien, 30000);
            Timer timerCumBerXuatHien = new Timer();
            TimerTask ttCumBerXuatHien = new TimerTask() {
                public void run()
                {
                    ...
                    maps[idMap].area[IDZONE].bossMap.add(_rBlack);
                    maps[idMap].area[IDZONE].loadBossNoPet(_rBlack);
                    Util.log("INIT CUMBER XONG KHU " + IDZONE);
                    timerBLACKXuatHien.cancel();
                };
            };
            timerCumBerXuatHien.schedule(ttCumBerXuatHien, 30000);
...
//                    short ySo0 = (short)240;
            ...
            TimerTask ttFIDE = new TimerTask() {
                public void run()
                {
                    ...
                    Service.gI().initSuperZou();
                    Service.gI().initCumBer();
                    timerFIDE.cancel();
                };
            };
